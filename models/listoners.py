...
def res_partner(env, model_name, record_id, vals):
	...
	if kb_username != '' and kb_password != '' and api_url != '':
		if api_url[-1] =='/':
			api_url = api_url[0:-1]
		...
		r = requests.get(
			api_url+'/1.0/kb/accounts?externalKey=%s&accountWithBalance=false&accountWithBalanceAndCBA=false&audit=NONE' %(record_id), 
	    	headers=get_headers, 
	    	auth=(kb_username, kb_password)
	    	)
...
def event(env, model_name, record_id, vals):
	...
	if kb_username != '' and kb_password != '' and api_url != '':
		...
		r = requests.post(
			api_url+'/1.0/kb/invoices/charges/%s?payInvoice=false&autoCommit=true' %(rec.killbill_id), 
			headers=post_headers, 
			params=params, 
			data=data, 
			auth=(kb_username, kb_password))
		...
		url = api_url+'/1.0/kb/invoices/%s' %(r.json()[0]['invoiceId'])

		r = requests.get(url, headers=get_headers, params=params, auth=(kb_username, kb_password))
